...
export async function signIn(
  ...
  // Do not block the tap on sign-out. A 10s hung /api/auth/sign-out on a
  // phone looks like a dead button. Best-effort clear still runs.
  void runPreSignInSignOut({
  ...
  if (!inLivePreview() && (providerId === "google" || providerId === "grok-google")) {
    ...
    if (error) {
      throw new Error(
        error.message && error.message !== "Failed to fetch"
          ? error.message
          : "Couldn't start Google sign-in from this site yet.",
      );
    }
    if (data?.url) {
    ...
    throw new Error("Couldn't start Google sign-in from this site yet.");
  }
  ...
  if (error) {
    throw new Error(
      error.message && error.message !== "Failed to fetch"
        ? error.message
        : "Couldn't start Google/X sign-in from this site yet.",
    );
  }
  if (data?.url) {
    window.location.assign(data.url);
  ...
  throw new Error("Couldn't start Google/X sign-in from this site yet.");
}
